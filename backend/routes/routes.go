...
func Setup(cfg *config.Config) *gin.Engine {
	...
	api := r.Group("/api/v1")
	{
		...
		vpn := protected.Group("/vpn")
		{
			...
			vpn.GET("/production-plans/:protocol", vpnHandler.ProtocolProductionPlan)
			vpn.POST("/config-preview", vpnHandler.PreviewProtocolConfig)
			vpn.GET("/:protocol/instances", vpnHandler.ListLegacyInstances)
			vpn.POST("/:protocol/instances", vpnHandler.CreateLegacyInstanceDraft)
			vpn.POST("/:protocol/instances/:id/apply", vpnHandler.ApplyLegacyInstance)
			...
			openvpn := vpn.Group("/openvpn")
			{
				...
				openvpn.POST("/instances/:id/apply", openVPNHandler.ApplyRuntime)
